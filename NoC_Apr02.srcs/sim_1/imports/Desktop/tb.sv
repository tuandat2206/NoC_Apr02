...
module tb;  
  ...
  task read();
    if (op_read == op_count) begin
      $display("All packet has been read");
    end else if (w_valid_pe_io == 1'b1) begin
      pkt_rcv = new();
  ...
  task write();
      if (op_count == op_write) begin
        $display("Number of send-packet is enough");
        r_valid_pe_io = 1'b0;
        r_data_pe_io = 1'b0;
      end
      else if (op_count > op_write && r_ready_pe_io == 1'b1) begin
        take_id = 0;
        r_valid_pe_io = 1'b0;
        for (id_pkt = 0; id_pkt < 7; id_pkt = id_pkt + 3'b001) begin
          if (flag_id[id_pkt] == 1'b0 && id_pkt !== id_read) begin
            pkt_snd = new();
            pkt_snd.id = id_pkt;
            pkt_snd.randomize();
            pkt_snd.concat();
      ...
      end else $display("Switch is not ready to take packet");
    endtask
  ...
  initial begin
    ...
    $display("No. of clk read-packet is read before: %d", written_before);
    $display("========================================================");
    $stop;
  end
  
  always @(op_write or op_read or read10time) begin
    if (op_write == op_count && op_read == op_count) -> done;
    if (read10time == 10) -> done;
  end
